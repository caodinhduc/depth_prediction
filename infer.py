...
class InferenceHelper:
    def __init__(self, dataset='kitti', device='cuda:0', checkpoint='./checkpoints/10_latest.pt'):
        ...
        if dataset == 'nyu':
            ...
            print('inference with kitti ...')
            self.min_depth = 1e-3
            self.max_depth = 80
            self.saving_factor = 128
...
if __name__ == '__main__':
    ...
    # PREDICTION FOR BENCHMARKING IMAGES
    ...
    for idx, path in enumerate(list_images):
        ...
        final = (pred.squeeze() * 256).astype('uint16')
        Image.fromarray(final).save(os.path.join(path.split('/')[0], 'ffc_before_conv', path.split('/')[1]))
        print('saved image {}'.format(idx))
