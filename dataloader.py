...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            # print('image after crop: ', image.size)
            image = np.asarray(image, dtype=np.float32) / 255.0

            # print('image after crop: ', image.shape)
    ...
    def random_crop(self, img, depth, height, width):
        assert img.shape[0] >= height, 'shape_0: {}'.format(img.shape[0])
        assert img.shape[1] >= width, 'shape_1: {}'.format(img.shape[1])
