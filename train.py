    ...
    for epoch in range(args.epoch, epochs):
        ...
        for i, batch in tqdm(enumerate(train_loader), desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Train",
            ...
            if random.random() > 0.6:
                r = random.random()
                img = graft_data(img, r)
                depth = graft_data(depth, r)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--validate-every', '--validate_every', default=5000, type=int, help='validation period')
    ...
    parser.add_argument("--resume", default='checkpoints/11_FFC_lastest.pt', type=str, help="Resume from checkpoint")
